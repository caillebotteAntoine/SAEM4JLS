...
run <- function(model, parameter0, init.options = list(), SAEM.options = list(), load.options = list(),  verbatim = 3)
{
  var0 <- init.SAEM(model, x0 = init.options$x0, sd = init.options$sd)

  do.call(load.SAEM, c(list(model), load.options, list(env = environment() )) )
