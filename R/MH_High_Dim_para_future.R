...
#'  plot(nrow = 2)
MH_High_Dim_para_future <- function(niter, x0, sd, loglik, ..., verbatim = F, cores = 1)
{
  ...
  f <- function(k)
  {
    ...
    if(verbatim)
      attr(res, 'value') <- value#as.data.frame(value, row.names = F)
  ...
  # === Acceptation rate === #
  x@acceptation <- res %>% sapply(function(x) attr(x, 'acceptation')) %>%
    matrix(nrow = niter) %>% apply(1, sum)

  if(verbatim)
    x@chain <- do.call(rbind , res %>% lapply(function(x) attr(x, 'value'))) # %>% as.data.frame

  return(SAEM4JLS::link(x0, x) )
}
# MH_High_Dim_para_future <- function(niter, x, sd, loglik, ..., verbatim = F, cores = 1)
# {
#   #check that x have a good shape for the high dimension
...
#   x <- res %>% sapply(function(x) as.numeric(x)) %>% matrix(ncol = dim) %>% t
#
#   attr(x, 'acceptation') <- res %>% sapply(function(x) attr(x, 'acceptation')) %>%
#     matrix(nrow = niter) %>% apply(1, sum)  %>%
