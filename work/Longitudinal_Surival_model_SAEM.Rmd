...
data <- JLS_data(G = 20, ng = 8, time = t, fct = m, param = param)
...
{
  lbd <- function(t,g) t^{b-1} * exp(alpha*m(t, eta[g], phi[g,, drop = F]))
...
S <- fct_vector(function(eta, phi, ...) mean((Y - get_obs(data, eta = eta, phi = phi) )^2 ),
                ...
                function(eta, phi, gamma, a, b, alpha)                        #12
                  uniroot(function(beta)zeta.der(beta, eta, phi, gamma, a,b,alpha), 
                          lower = -100, upper = 100, tol = 1e-3)$root,
...
Z <- list(eta = list( matrix(rep(0, G*ng), ncol = 1) ),
          ...
          gamma = list(matrix(rep(0, G), ncol = 1)),#matrix(rnorm(G, 0, para$nu2), ncol = 1) ),
...
sim <- function(niter, h, Phih, eta, phi, gamma, a, b, alpha, verbatim = F)
{
  ...
  b <- list(MH_High_Dim_future(niter, b[[1]],     sd = .5, loglik.b, a[[1]],
                               Phih, cores = 1, verbatim = verbatim ))
  alpha <- list( MH_High_Dim_future(niter, alpha[[1]], sd = .5, loglik.alpha,
                                    Phih, cores = 1, verbatim = verbatim ))

  list(eta = eta, phi = phi, gamma = gamma, a = a, b = b, alpha = alpha)
}

#res <- simulation_test(sim, Phi, param, 100, Z) %>% lapply(function(z)plot(z[[1]], nrow = 2))
