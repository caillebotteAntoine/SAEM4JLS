...
sim <- function(niter, h, Phih, eta, phi, verbatim = F)
{
  ...
  eta <- 1:M %>% lapply( function(i)
    MH_High_Dim_para_future(niter, eta[[i]], sd.eta(h) , loglik.eta, phi[[i]], Phih, 
                            cores = 1, verbatim = verbatim))
  
  phi <- 1:M %>% lapply( function(i)
    MH_High_Dim_para_future(niter, setoffset(phi[[i]],-Phih%a%4), sd.phi(h), loglik.phi, eta[[i]], Phih, 
                            cores = 1, verbatim = verbatim )) %>%
...
niter <- 300
correction.phase <- 50
MH.iter <- 20#function(k) ifelse(k<correction.phase, 20, 10)

sd.eta <- function(k) .03
sd.phi <- function(k) c(.05, .05, .01)
