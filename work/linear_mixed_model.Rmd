...
```{r MH}
transi.z <- function(x) rnorm(length(x), x, 0.1)
...
Z.MH <- MH_High_Dim(2000, Z.MH, transi.z, loglik.z, do.call(Phi, param), verbatim = T)

plot(Z.MH)

# burn in des premières valeurs
z <- MH_burnin(Z.MH, 1000)
#Moyennes et variance sachant Y de Z
est <- attr(z, 'value') %>% group_by(id) %>% summarise(mu = mean(x), o2 = var(x))

o2.true <- (sum(tj^2)/param$sigma2 + 1/param$gamma2)^-1
#mu.true <- 1:n %>% sapply( function(i) o2.true/param$sigma2 * sum( (Y[i,] -param$a - param$b*tj)*tj) )

mu.true <- dt %>% group_by(id) %>% summarise(mu = o2.true/param$sigma2* sum((obs - param$a -param$b*t)*t)) %>% { .$mu }
...
niter <- 200
res <- SAEM(niter, 100, u, para, Phi, S, list(Z = ZgivenY), sim, maxi, eps = 1e-2, verbatim = T)
