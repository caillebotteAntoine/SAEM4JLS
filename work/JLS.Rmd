...
zeta.A <- function(alpha, beta, phi1, phi2, phi3, a,b) -zeta(alpha, beta, phi1, phi2, phi3, a,b)/(n*G)
...
zeta.der.A <- function(alpha, k, beta, phi1, phi2, phi3, a, b)
{
  ...
  -(sum(link.value) - b*a^-b*sum(P1*P2))/(n*G)
}
...
res <- SPGD(100, theta0 = 0.4,
      step = 1e-2,
      zeta.der.A, 1, zeta.A,
      param$beta,  var.true$phi1, var.true$phi2, var.true$phi3, var.true$a,var.true$b, verbatim = T)

plot(res, seq(0.45,0.52, by = 0.01), zeta.A, zeta.der.A, 1, param$beta,  var.true$phi1, var.true$phi2, var.true$phi3, var.true$a,var.true$b)


res <- SPGD(100, theta0 = 5,
      step = 1e-2,
      zeta.der.B, 1, zeta.B,
      param$alpha,  var.true$phi1, var.true$phi2, var.true$phi3, var.true$a,var.true$b, verbatim = T)

plot(res, seq(9,11, by = 0.1), zeta.B, zeta.der.B, 1, param$alpha,  var.true$phi1, var.true$phi2, var.true$phi3, var.true$a,var.true$b)
```
...
# saem
```{r saem}
exclude.sim = c('a', 'b') ; exclude.max = c('alpha', 'beta')
