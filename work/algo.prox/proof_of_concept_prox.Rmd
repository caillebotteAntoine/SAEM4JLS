...
```{r SPGD res}
l <- function(beta, slope, param, ... ) 1/(2*param$sigma2) * sum( (Y - as.vector(beta %*% tU) - slope %*% ttime )^2 ) 
grad.li <- function(beta, i, slope, ...) -1/param$sigma2 * sum(Y[i,] - sum(beta * U[i,]) - slope[i]*time) * U[i,]
```


```{r SPGD test, eval=FALSE, include=FALSE}

res <- SPGD(500, theta0 = c(0,0),
            step = 5e-3,# mini.batch.size = n/2,
...
theta0 = c(1,1)
iteration <- length(x)
theta <- matrix(rep(theta0, iteration), ncol = length(theta0))
beta[,i] <- x

plot(beta[,1], sapply(1:100, function(i) l(beta[i,], slope, param)))

plot(attr(res, 'f.value'))
plot(attr(res, 'gradf.value')[,1])
plot(attr(res, 'gradf.value')[,2])

l(param$beta, slope, param)
l(res, slope, param)
